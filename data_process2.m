clear; close all; clc
% 读取文件
load('noemergency_speed1_v.mat')
%watch2_rou = [rou{:,2}];watch3_rou = [rou{:,3}];watch4_rou = [rou{:,4}];
watch2_v = [v{:,2}];watch3_v = [v{:,3}];watch4_v = [v{:,4}];
%watch2_q = [q{:,2}];watch3_q = [q{:,3}];watch4_q = [q{:,4}];
% watch2_v(60:end) = watch2_v(60:end)+5;
%watch3_v(160:end) = watch3_v(160:end)-5;
%watch3_v(168) = watch2_v(168)+20;
%watch4_v(160) = 30;watch4_v(159) = 30;
% watch4_v(102:160) = watch4_v(102:160)+randi([-4,3],1,1);
% watch4_v(180:end) = watch4_v(180:end)+randi([-3,3],1,1);

indicate2_v = 80./(watch2_v+30); indicate3_v = 80./(watch3_v+30); indicate4_v = 80./(watch4_v+30);
% indicate3_v(140:end) = indicate3_v(140:end)+randi([0,1],1,1);


tblock = length(find(indicate2_v>2.0))+length(find(indicate3_v>2.0))+length(find(indicate4_v>2.0));
percent = tblock/(3*length(indicate2_v));
...
ylabel('拥堵指标');
xlabel('时间(s)');
title('已开启应急车道的拥堵指标变化');
exportgraphics(h, 'noemergency_indecate.png', 'BackgroundColor', 'none');

hold on;
% subplot(1, 3, 1); % 2行2列的网格，绘制第1个子图
% plot(watch2_v,watch2_q,'o');
% [fitresult_qv, gof] = fit(watch2_v', watch2_q', 'poly8');
% hold on;
% plot(fitresult_qv, 'r-');
% xlabel('速度(m/s)');
% ylabel('交通流量(veh/s)');
...
% title('q-v');
% 
% subplot(1, 3, 2); % 2行2列的网格，绘制第1个子图
% plot(watch2_rou,watch2_q,'o');
% [fitresult_qrou, gof] = fit(watch2_rou', watch2_q', 'poly8');
% hold on;
% plot(fitresult_qrou, 'r-');
% xlabel('密度veh/m)');
% ylabel('交通流量(veh/s)');
% ylim([1 max(watch2_q)]);
% title('rou-v');
% 
% subplot(1, 3, 3); % 2行2列的网格，绘制第1个子图
% plot(watch2_rou,watch2_v,'o');
% [fitresult_rouv, gof] = fit(watch2_rou', watch2_v', 'poly8');
% hold on;
% plot(fitresult_rouv, 'r-');
% xlabel('密度(veh/m)');
% ylabel('速度(m/s)');
% ylim([1 max(watch2_v)]);
% title('rou-v');
...
% plot(watch4_q, 'g-');
% legend("监控2","监控3","监控4");
% title('未开启应急车道的交通流量(veh/s)');
% exportgraphics(h, 'noemergency_speed1_q.png', 'BackgroundColor', 'none');
...
xlabel('时间(s)');
title('未开启应急车道后的速度变化');
exportgraphics(h, 'noemergency_v.png', 'BackgroundColor', 'none');
